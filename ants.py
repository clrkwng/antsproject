    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 9
                #Handles if the placed insect is container
                if insect.is_container and insect.can_contain(self.ant):
                    insect.contain_ant(self.ant)
                    self.ant = insect
                #Handles if the ant here is container
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if isinstance(insect, QueenAnt):
                if not insect.is_first:
                    QueenAnt.queen_instances -= 1
                else:
                    return None
...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        # BEGIN EC
        "*** YOUR CODE HERE ***"
        if self.travel_direction == "backwards" and not self.already_scared:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive, min_range = 0, max_range = float('inf')):
        ...
        place = self.place
        while place and place is not hive:
            if place.bees:
                distance = int(place.name[len(place.name)-1])
    ...
    def action(self, colony, min_range = 0, max_range = float('inf')):
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        self.armor = armor
        if not QueenAnt.queen_instances:
            self.is_first = True
        else:
            self.is_first = False
        QueenAnt.queen_instances += 1
    ...
    def action(self, colony):
        ...
        if not self.is_first:
            ...
            place_to_buff = self.place.exit
            while place_to_buff:
                if place_to_buff.ant:
                    if isinstance(place_to_buff.ant, BodyguardAnt):
                        if place_to_buff.ant.contained_ant and place_to_buff.ant.contained_ant not in QueenAnt.buff_ants:
                            place_to_buff.ant.contained_ant.damage *= 2
                            QueenAnt.buff_ants.append(place_to_buff.ant.contained_ant)
                    if place_to_buff.ant not in QueenAnt.buff_ants:
                        place_to_buff.ant.damage *= 2
                        QueenAnt.buff_ants.append(place_to_buff.ant)
                place_to_buff = place_to_buff.exit
...
def make_scare(action, bee):
    ...
    def scare_action(colony):
        bee.travel_direction = "backwards"
        action(colony)
    return scare_action
    # END Problem EC
